...
async def get_figure_cards(game_id: int, player_id: int, db: Session = Depends(get_db), repo: FigureCardsRepository = Depends()):
...
async def get_figure_cards(game_id: int, player_id: int, card_id: int, db: Session = Depends(get_db), repo: FigureCardsRepository = Depends()):
    return repo.get_figure_cards_by_id(game_id, player_id, card_id, db)
